...
#         st.write(random_symbols_30[i] + ": " + str(weights_max_sr_perc_30[i]) + "%")
    ...
    while True:
        ...
        # Check if any price data has missing values
        if not price_data_10_symbols_personalizado.isnull().values.any() and \
                not price_data_20_symbols_personalizado.isnull().values.any() and \
                not price_data_30_symbols_personalizado.isnull().values.any():
            break 
