...
def main():
    ...
    st.title("Ahora lo importante, ¿cuánto habría rendido tu capital si lo invertías con nosotros en los últimos años?")
    st.subheader("¡Abajo te mostramos eso!")
    
    # Define layout
    layout = go.Layout(
    title=dict(
        text='Retorno acumulado por portafolio',
        x=0.3, y=0.9,
        font=dict(size=24, color='#666666')  # Gray color for the title
    ),
    xaxis=dict(
        title='Fecha',
        tickformat='%Y-%m-%d',
        showgrid=False,
        linecolor='#666666',  # Gray color for the axis line
        tickfont=dict(color='#666666')  # Gray color for the tick labels
    ),
    yaxis=dict(
        title='Retorno acumulado en porcentaje %',
        showgrid=False,
        linecolor='#666666',  # Gray color for the axis line
        tickfont=dict(color='#666666')  # Gray color for the tick labels
    ),
    ...
    initial_investment = st.number_input("Ingrese el monto de su inversión inicial:", min_value=0.01)

    if initial_investment:
        # Calculate simulated investment returns based on user input and cumulative portfolio returns
        simulated_returns_10 = initial_investment * (1 + cumulative_returns_10 / 100)
        simulated_returns_20 = initial_investment * (1 + cumulative_returns_20 / 100)
        simulated_returns_30 = initial_investment * (1 + cumulative_returns_30 / 100)

        # Create traces for simulated portfolio returns
        sim_trace_10 = go.Scatter(x=cumulative_returns_10.index, y=simulated_returns_10, mode='lines', name='Simulado (10 acciones)', line=dict(color='rgb(31, 119, 180)', width=2))
        sim_trace_20 = go.Scatter(x=cumulative_returns_20.index, y=simulated_returns_20, mode='lines', name='Simulado (20 acciones)', line=dict(color='rgb(255, 127, 14)', width=2))
        sim_trace_30 = go.Scatter(x=cumulative_returns_30.index, y=simulated_returns_30, mode='lines', name='Simulado (30 acciones)', line=dict(color='rgb(44, 160, 44)', width=2))

        # Create data list for the simulated traces
        sim_data = [sim_trace_10, sim_trace_20, sim_trace_30]

        # Define layout for the simulated plot
        sim_layout = go.Layout(
            title=dict(
                text='Simulación de retorno acumulado por portafolio',
                x=0.5, y=0.9,
        ...
        # Create figure for the simulated plot
        sim_fig = go.Figure(data=sim_data, layout=sim_layout)

        # Update figure aesthetics
        sim_fig.update_traces(marker=dict(size=5))

        # Display the simulated plot using Streamlit
        st.plotly_chart(sim_fig)


    
    # Add a title
    st.title("Antes de irte, participa de nuestra encuesta")

    # Add a description with a hyperlink
    st.markdown("¡Tu opinión es importante para nosotros! [Participa aquí](https://forms.gle/YhXpdo1cs59EZ1948)")
